...
function renderResult(
  ...
  results.forEach((item) => {
    ...
    downloadExpand.addEventListener("click", () =>
      downloadIconMyGifo(img, gifContainer)
    );
...
function imgHoverComponentFactory(
  item,
  expandir,
  agregarFavoritos,
  eliminarFavoritos,
  ...
  violetDownloadHover.addEventListener("click", () =>
    downloadIconGifo(item, violet)
  );
...
function eliminarFavoritos(item) {
  ...
  localStorage.setItem(
    "listFavorites",
    JSON.stringify(
      localStorageFavorites.filter((favorite) => favorite.id !== item.id)
    )
  );
}
...
async function palabrasTendencia() {
  const tendencias =
    "https://api.giphy.com/v1/trending/searches?api_key=EjzvMRueNdiAkT3CvCjx0kOjl8qGzxLM";
...
function repeatCapture() {
  ...
  video.style.display = "none";
  titleCrear.className = "titleCrear";
...
async function subirGifo() {
  ...
  if (resUpload.meta.status === 200) {
    ...
    const resultadoUpload = await fetch(
      `https://api.giphy.com/v1/gifs/${resUpload.data.id}?api_key=EjzvMRueNdiAkT3CvCjx0kOjl8qGzxLM`
    );
    const res = await resultadoUpload.json();
    console.log(res);
    const aLink= res.data.url;
    const templateFinal = uploadedOverlay(template, aLink)
...
function uploadedOverlay(uploadOverlay, aLink){
  ...
  linkMyGifo.addEventListener("click", () => redirigir(Link))
  return uploadOverlay
...
function eliminarMisGifos(item) {
  localStorage.setItem(
    "listMisGifos",
    JSON.stringify(
      localStorageMisGifos.filter((migifo) => migifo.id !== item.id)
    )
  );
}
